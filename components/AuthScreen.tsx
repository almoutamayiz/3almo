...
import { 
    ...
    Gamepad2, Eye, EyeOff, Ticket, BrainCircuit, Sparkles, 
    ChevronDown
} from 'lucide-react';
...
const AuthScreen: React.FC<AuthScreenProps> = ({ onLogin, onRegister }) => {
  ...
  return (
    // التغيير الجوهري هنا: استبدال overflow-hidden بـ overflow-y-auto للسماح بالسحب
    // وإضافة touch-pan-y لتحسين استجابة اللمس
    <div className="relative flex flex-col items-center justify-start h-full w-full bg-[#020202] font-cairo pb-10 overflow-x-hidden overflow-y-auto scroll-container selection:bg-brand selection:text-black touch-pan-y">
      <PrivacyModal isOpen={showPrivacy} onClose={() => setShowPrivacy(false)} />
      
      {/* --- Advanced Aesthetic Background Layers (GPU Accelerated) --- */}
      <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
          {/* Animated Glow Blobs - Added transform-gpu for performance */}
          <div className="absolute top-[-10%] right-[-10%] w-[500px] h-[500px] bg-brand/10 blur-[80px] rounded-full animate-float-slow transform-gpu will-change-transform"></div>
          <div className="absolute bottom-[10%] left-[-5%] w-[400px] h-[400px] bg-brand/5 blur-[60px] rounded-full animate-float-slow transform-gpu will-change-transform" style={{ animationDelay: '-5s' }}></div>
          
          {/* Background Waves (SVGs) */}
          <div className="absolute bottom-0 left-0 w-full opacity-[0.08] animate-wave-move whitespace-nowrap transform-gpu">
      ...
      <div className="relative z-10 w-full max-w-lg px-5 pt-10 flex flex-col items-center gap-8 min-h-screen">
        
        {/* Header Section */}
        <div className="text-center animate-fadeIn transform-gpu">
        ...
        {/* Auth Card - Reduced Blur for Performance */}
        <div className="w-full bg-neutral-900/60 backdrop-blur-xl border border-white/5 rounded-[2.5rem] p-6 shadow-2xl relative overflow-hidden group transform-gpu">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 relative z-10">
                {!isLogin && (
                    <div className="space-y-1.5">
                        <label className="text-[9px] text-gray-500 font-black uppercase mr-2 tracking-widest transition-colors">الاسم الكامل</label>
                        <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="اسمك الكامل..." className="w-full bg-black/40 border border-white/10 rounded-xl py-3.5 px-6 text-sm font-bold text-white outline-none focus:border-brand/40 transition-all shadow-inner" required={!isLogin} />
                    </div>
                )}
                
                <div className="space-y-1.5">
                    <label className="text-[9px] text-gray-500 font-black uppercase mr-2 tracking-widest transition-colors">البريد الإلكتروني</label>
                    <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="example@mail.com" className="w-full bg-black/40 border border-white/10 rounded-xl py-3.5 px-6 text-sm font-bold text-white outline-none focus:border-brand/40 transition-all shadow-inner" required />
                </div>
                
                <div className="space-y-1.5">
                    <label className="text-[9px] text-gray-500 font-black uppercase mr-2 tracking-widest transition-colors">كلمة المرور</label>
                    <div className="relative">
                        <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className="w-full bg-black/40 border border-white/10 rounded-xl py-3.5 px-6 text-sm font-bold text-white outline-none focus:border-brand/40 transition-all shadow-inner" required />
                ...
                {!isLogin && (
                    <div className="space-y-1.5 animate-slideIn">
                        ...
                        <input type="text" value={referralCode} onChange={(e) => setReferralCode(e.target.value)} placeholder="كود صديقك لربح النقاط..." className="w-full bg-brand/5 border border-brand/10 rounded-xl py-3.5 px-6 text-brand text-sm font-black outline-none focus:border-brand/30 transition-all uppercase tracking-widest" />
                    </div>
                )}
        ...
        {/* Smart Parser Demo */}
        <div className="w-full space-y-4 animate-slideUp relative z-10 transform-gpu">
        ...
        <div className="w-full space-y-6 pt-4 relative z-10">
            ...
            <div className="grid grid-cols-2 gap-3 w-full">
                {features.map((f, i) => (
                    <div key={i} className="bg-neutral-900/60 backdrop-blur-md border border-white/5 p-5 rounded-2xl flex flex-col items-center gap-3 text-center shadow-lg group hover:border-brand/20 transition-all duration-500 transform-gpu">
        ...
        {/* Footer */}
        <footer className="w-full mt-6 pt-10 border-t border-white/5 flex flex-col items-center gap-6 pb-10 relative z-10">
