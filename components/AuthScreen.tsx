...
const AuthScreen: React.FC<AuthScreenProps> = ({ onLogin, onRegister }) => {
  ...
  return (
    // FIX: Changed min-h-screen to h-screen and added overflow-y-auto to handle scrolling internally
    <div className="h-screen w-full bg-[#020202] font-cairo overflow-y-auto overflow-x-hidden selection:bg-brand selection:text-black relative">
      <PrivacyModal isOpen={showPrivacy} onClose={() => setShowPrivacy(false)} />
      
      {/* --- Advanced Aesthetic Background Layers --- */}
      <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
          {/* Animated Glow Blobs */}
          <div className="absolute top-[-10%] right-[-10%] w-[500px] h-[500px] bg-brand/10 blur-[120px] rounded-full animate-float-slow"></div>
          <div className="absolute bottom-[10%] left-[-5%] w-[400px] h-[400px] bg-brand/5 blur-[100px] rounded-full animate-float-slow" style={{ animationDelay: '-5s' }}></div>
      ...
      {/* Wrapper to ensure content centers but allows scroll */}
      <div className="w-full min-h-full flex flex-col items-center justify-start py-10">
        <div className="relative z-10 w-full max-w-lg px-5 flex flex-col items-center gap-8">
            ...
            {/* Auth Card */}
            <div className="w-full bg-neutral-900/40 backdrop-blur-2xl border border-white/5 rounded-[2.5rem] p-6 shadow-2xl relative overflow-hidden group">
                <div className="absolute top-0 right-0 w-32 h-32 bg-brand/5 blur-3xl rounded-full -translate-y-16 translate-x-16 group-hover:bg-brand/10 transition-colors"></div>
            ...
            <div className="w-full space-y-4 animate-slideUp relative z-10">
                ...
                <div className="w-full bg-neutral-900/30 backdrop-blur-3xl border border-white/5 rounded-[2.5rem] p-4 shadow-2xl relative overflow-hidden">
            ...
            <div className="w-full space-y-6 pt-4 relative z-10">
                ...
                <div className="grid grid-cols-2 gap-3 w-full">
                    {features.map((f, i) => (
                        <div key={i} className="bg-neutral-900/40 backdrop-blur-md border border-white/5 p-5 rounded-2xl flex flex-col items-center gap-3 text-center shadow-lg group hover:border-brand/20 transition-all duration-500">
            ...
            {/* Footer */}
            <footer className="w-full mt-6 pt-10 border-t border-white/5 flex flex-col items-center gap-6 relative z-10">
