...
const CalculatorScreen: React.FC<CalculatorScreenProps> = ({ onBack }) => {
  ...
  const handleDownloadResults = () => {
      ...
      let content = `Calculated by almoutamayiz .\n\n`;
      ...
      const trans: Record<string, string> = {
          ...
          math: 'Mathematics',
          sport: 'Physical Education (Sport)',
          amazigh: 'Amazigh Language'
      };

      BAC_SUBJECTS.forEach(sub => {
          const mark = marks[sub.id] || '0';
          content += `${trans[sub.id].padEnd(25)} : ${mark}\n`;
      });

      if (hasSport) {
          const mark = marks['sport'] || '0';
          content += `${trans['sport'].padEnd(25)} : ${mark}\n`;
      }
      if (hasAmazigh) {
          const mark = marks['amazigh'] || '0';
          content += `${trans['amazigh'].padEnd(25)} : ${mark}\n`;
      }

      content += `\n` + `-`.repeat(40) + `\n`;
      content += `FINAL AVERAGE : ${average}\n`;
      content += `-`.repeat(40) + `\n`;
      ...
      link.href = url;
      link.download = `marks_report_${average.replace('.', '_')}.txt`;
  ...
  return (
    // FIX: Use h-screen and overflow-y-auto
    <div className="h-screen w-full bg-black text-white p-4 sm:p-6 animate-fadeIn pb-24 overflow-y-auto font-cairo">
